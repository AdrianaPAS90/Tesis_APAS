...
#  (alpha_0-1)*log(alpha_d_sim)+alpha_d_sim*beta_0
  ...
  # (c)
  #(alpha_d_sim) * (sum( log( d[los.unos,"duration"]+beta_theta_sim ) ) + sum( log( theta_sim[los.unos,1,1] ) ))
  
  # (d)
#  (2*alpha_d_sim) * sum( log( d[los.no.unos,"duration"] + d[los.no.unos-1,"duration"] + beta_theta_sim) ) + sum( log( theta_sim[los.no.unos,1,1] ) )
  ...
  uni <- uni.slice(alpha_d_sim, function(alpha_d) -(I*log(gamma(alpha_d + alpha_theta_sim))+n.sum*log(gamma(2*alpha_d + alpha_theta_sim))) +
                     ...
                     ((alpha_d) * (sum( log( d[los.unos,"duration"]+beta_theta_sim ) ) 
                                  + sum( log( theta_sim[los.unos,1,1] ) ) ) )+ 
                     ((2*alpha_d) * (sum( log( d[los.no.unos,"duration"] + d[los.no.unos-1,"duration"] + beta_theta_sim) ) 
                                     + sum( log( theta_sim[los.no.unos,1,1] ) ))),
