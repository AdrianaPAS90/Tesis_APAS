...
#df_aux<-read.csv("datos_agregados.csv", header=TRUE)
#alpha_d <- 1
#alpha_theta <- 2
#t<-c(2,4,6,8,10)
#t1<-t(t)
...
bayesmpp_alpha_d <- function(x0=1){
  ...
  g<-function(alpha_d,d_var= 5,alpha_theta=2,beta_theta=1, theta=3, alpha_0=2,beta_0=0.3){
    ...
    a<- adi*((alpha_0-1)*(log(alpha_d))-(alpha_d*beta_0)-lgamma(alpha_d + alpha_theta))
    v_1<-0
    v_2<-0
    for(i in 1:I){
      v_1<-v_1 + v_2
      v_2 <- 0
      for(j in 2:T_n(i)){
        v_2 <- v_2 + (((alpha_d+alpha_theta)*log(d(i,j-1)+beta_theta))+(alpha_d*log(theta)))
      }
    }
    c<-v_1
    g<- a + c
 ...
 bad<-uni.slice(x0=2, g, w=1, m=10,lower=-Inf, upper=Inf, gx0=NULL)
 return(bad)
}
