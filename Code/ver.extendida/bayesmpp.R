...
T_n<-function(i){
  nrow(datos[datos$paciente==i, ])  
}


d<-function(i,j){
  datos$duration[datos$paciente==i][j]
}

c<-function(i,j){
  datos$costs[datos$paciente==i][j]
}
...
bayesmpp <- function(alpha_0 =2,beta_0 = 0.3,  d, c, n, M){
	...
	for(m in 1:M){
		...
		#Simular la final completa de parámetros y variables que dependen de costos o duraciones anteriores.
		j <- 1
		for (j in 1:n){
