gamma2_slice<-function(alpha_gamma_sim,beta_gamma_sim,
                       d,c){
  ...
  uni_gamma2 <- uni.slice(gamma_sim, function(gamma2) -(d[los.no.unos,"duration"]+  d[los.no.unos-1,"duration"] + alpha_gamma_sim)*log(gamma2[los.no.unos.c,1,1])
                          -(beta_gamma_sim/gamma2[los.no.unos.c,1,1])+ ((c[los.no.unos.c,"costs"])/gamma2[los.no.unos.c,1,1])^(d[los.no.unos-1,"duration"]) +((c[los.no.unos.c,"costs"]/gamma2[los.no.unos.c,1,1])^(d[los.no.unos,"duration"])),
                          w=1, m=Inf, lower=0, upper=+Inf)
  
  
  gamma_sim <- attr(uni_gamma2,"uni.slice.evals")
