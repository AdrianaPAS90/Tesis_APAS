...
rm(list=ls())
tabla_inicial_NHS<-read.csv("./Diabetes.Marked/diabetes_marked_datos_sim.csv", header = TRUE)
...
head(data_agg)
colnames(data_agg) <- c("contador","paciente","num.cambio","presc.code",
                        "presc.cost","duration","costs" )

write.csv(data_agg,"./Diabetes.Marked/datos_agregados.csv")
